...
def run_multitask_svr():
    """
    This example demonstrates the robustness of Multi-Task Support Vector Regression
    (using epsilon-insensitive loss) paired with Adaptive Lasso penalty, 
    especially in the presence of heavy outliers in the target variables.
    """
    seed = 42
    ...
    mt_lasso = MultiTaskRegressor(loss="squared_error", penalty="l1", alpha=1.1)
    ...
    mt_adaptive_svr = MultiTaskRegressor(
        loss="epsilon_insensitive", epsilon=1, penalty="l1", alpha=1.1, adaptive=True,
    )
    ...
    plots = [
        ...
        (mt_lasso.coef_.T, "Multi-Task Lasso\n(Corrupted by Outliers)"),
        (mt_svr.coef_.T, "Multi-Task SVR\n(Robust to Outliers)"),
        (mt_adaptive_svr.coef_.T, "Adaptive Multi-Task SVR\n(Robust & Strict Sparsity)")
    ]
    
    # Shared color scale based on true coefficients
