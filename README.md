...
- **Adaptive Penalties**: Single-target models support `adaptive=True`, scaling pure $L_1$ or pure $L_2$ penalties by inverse-magnitude weights derived from a previous model fit (e.g. Linear Regression or RidgeCV). For combined `l1_l2` penalties, adaptive weights are applied only to the $L_1$ component.
